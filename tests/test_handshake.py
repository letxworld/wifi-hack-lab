...
class TestHandshakeParsing:
    ...
    def test_extract_handshake_data_with_eapol(self, mock_rdpcap):
        ...
        class MockEAPOL(Packet):
            def __init__(self):
                self.key_descriptor_version = 2
                self.wpa_key_nonce = b'\x01' * 32
                self.wpa_key_mic = b'\x02' * 16

        class MockPacket(Packet):
            def __init__(self):
                self.addr2 = "AA:BB:CC:DD:EE:FF"
                self[Dot11EAPOL] = MockEAPOL()

            def haslayer(self, layer):
                return layer == Dot11EAPOL
    ...
    def test_extract_handshake_data_non_hex_nonce(self, mock_rdpcap):
        ...
        class MockEAPOL(Packet):
            def __init__(self):
                self.key_descriptor_version = 2
                self.wpa_key_nonce = b'\xff' * 32  # Non-printable
                self.wpa_key_mic = b'\x00' * 16
